...
class ApplicationApprovedConsumer(BaseKafkaConsumer):
    ...
    def handle(self, event: Dict[str, Any]) -> None:
        ...
        if not renter_id or not agent_id:
            logger.warning(
                "Missing renter_id or agent_id in ApplicationApproved event: %s", event
            )
            return

        if renter_id == agent_id:
            logger.info(
                "Renter and agent are the same person; skipping conversation creation."
            )
            return
        ...
        try:
            ...
            conversation = ConversationService.get_or_create_direct_conversation(
                user1_id=renter_id, user2_id=agent_id
            )
