...
class ListingFilter(django_filters.FilterSet):
    ...
    min_deposit = django_filters.NumberFilter(
        field_name="deposit_amount", lookup_expr="gte"
    )
    max_deposit = django_filters.NumberFilter(
        field_name="deposit_amount", lookup_expr="lte"
    )

    min_lease_term = django_filters.NumberFilter(
        field_name="lease_term", lookup_expr="gte"
    )
    max_lease_term = django_filters.NumberFilter(
        field_name="lease_term", lookup_expr="lte"
    )

    available_from = django_filters.DateFilter(
        field_name="available_date", lookup_expr="gte"
    )
    available_to = django_filters.DateFilter(
        field_name="available_date", lookup_expr="lte"
    )
    ...
    bedrooms = django_filters.NumberFilter(
        field_name="unit_ID__no_bedrooms", lookup_expr="gte"
    )
    bathrooms = django_filters.NumberFilter(
        field_name="unit_ID__no_bathrooms", lookup_expr="gte"
    )
...
class UnitFilter(django_filters.FilterSet):
    ...
    min_bedrooms = django_filters.NumberFilter(
        field_name="no_bedrooms", lookup_expr="gte"
    )
    max_bedrooms = django_filters.NumberFilter(
        field_name="no_bedrooms", lookup_expr="lte"
    )

    min_bathrooms = django_filters.NumberFilter(
        field_name="no_bathrooms", lookup_expr="gte"
    )
    max_bathrooms = django_filters.NumberFilter(
        field_name="no_bathrooms", lookup_expr="lte"
    )
...
class AgentFilter(django_filters.FilterSet):
    ...
    organization = django_filters.CharFilter(
        field_name="agent_organization", lookup_expr="icontains"
    )

    min_experience = django_filters.NumberFilter(
        field_name="agent_experience", lookup_expr="gte"
    )
    max_experience = django_filters.NumberFilter(
        field_name="agent_experience", lookup_expr="lte"
    )
