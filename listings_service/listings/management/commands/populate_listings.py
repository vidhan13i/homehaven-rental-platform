...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for k in range(1000):
            ...
            is_semi_furnished = (
                False if is_furnished else rng_unit.choice([True, False])
            )
            ...
            if unit_created:
                ...
                Images.objects.create(image_url=img_url, unit_ID=unit)
            ...
            listing, listing_created = Listing.objects.get_or_create(
                ...
                defaults={
                    ...
                    + timedelta(days=rng_unit.randint(5, 60)),
                    "publish_date": timezone.now().date()
                    - timedelta(days=rng_unit.randint(1, 15)),
                    "closing_date": timezone.now().date()
                    + timedelta(days=rng_unit.randint(60, 120)),
                    "lease_term": rng_unit.choice([6, 12, 24]),
                    "is_listing_verified": rng_unit.choice(
                        [True, True, False]
                    ),  # 66% verified
                    "unit_ID": unit,
                },
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully seeded {created_units} units and {created_listings} listings."
            )
        )
