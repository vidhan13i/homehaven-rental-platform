...
def rental_history(value):
    ...
    try:
        ...
        raise ValidationError(
            "move_in and move_out must be dates in YYYY-MM-DD format"
        )

    if move_out < move_in:
        raise ValidationError(
            "move_out date cannot be earlier than move_in date"
        )
def emergency_info(value):
     ...
     if not required_keys.issubset(value.keys()):
         raise ValidationError("emergency_info must contain name, email, phone and relationship"
                               )
