...
@extend_schema_view(
    ...
    create=extend_schema(
        ...
        tags=["Applications"],
        examples=[
            OpenApiExample("Create App", value={"status": "pending"}, request_only=True)
        ],
    ),
    update=extend_schema(summary="Update Application", tags=["Applications"]),
    partial_update=extend_schema(
        summary="Partially Update Application", tags=["Applications"]
    ),
...
class ApplicationViewSet(viewsets.ModelViewSet):
    ...
    def by_unit(self, request):
        ...
        if not unit_id:
            return Response(
                {"error": "unit_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    try:
        ...
        event = build_event(
            ...
            payload={
                ...
                "building_id": (
                    str(application.building_ID) if application.building_ID else None
                ),
