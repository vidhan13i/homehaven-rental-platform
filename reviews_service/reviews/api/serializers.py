...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Review
        fields = [
            'id', 'profile_ID', 'building_ID',
            'full_address', 'unit_no',
            'Title', 'Pros', 'Cons', 'Advice',
            'cleanliness_rating', 'garbage_management_rating',
            'neighbours_rating', 'water_supply_rating',
            'building_maintenance_rating', 'avg_rating',
            'move_in_date', 'move_out_date',
            'is_received_deposit', 'total_deposit', 'deposit_withheld',
            'is_pet_friendly',
            'starting_rent', 'ending_rent', 'rent_change',
    ...
    def validate(self, data):
        if data.get('move_in_date') and data.get('move_out_date'):
            if data['move_out_date'] < data['move_in_date']:
                raise serializers.ValidationError("Move-out date cannot be before move-in date")

        if data.get('deposit_withheld') and data.get('total_deposit'):
            if data['deposit_withheld'] > data['total_deposit']:
                raise serializers.ValidationError("Deposit withheld cannot exceed total deposit")
